...
func StoreTCPResult(timestamp int64, ip, port string, rtt float64, success bool) error {
	...
	if err != nil {
		log.Printf("Error preparing TCP result statement: %v", err)
		return err
	}
	defer stmt.Close()
	...
	if err != nil {
		log.Printf("Error executing TCP result statement: %v", err)
		// 如果插入失败，回滚事务
		tx.Rollback()
		return err
	}
	...
	// 提交事务
	if err := tx.Commit(); err != nil {
		log.Printf("Failed to commit TCP result transaction: %v", err)
		return err
	}

	log.Printf("Successfully inserted TCP result into ClickHouse: timestamp=%d, ip=%s, port=%s, rtt=%f", timestamp, ip, port, rtt)
	return nil
}
